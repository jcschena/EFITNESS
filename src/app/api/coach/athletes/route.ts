...
export async function GET(req: Request) {
  try {
    ...
    // 1. Obter atletas vinculados a este treinador
    const athletes = await db.all('SELECT id, name, level, weight, threshold_hr, threshold_pace, strava_connected FROM users WHERE coach_id = ? AND role = \'athlete\' ORDER BY name ASC', coachId);
    ...
    // 2. Obter as chaves de acesso deste treinador
    const accessKeys = await db.all('SELECT * FROM access_keys WHERE coach_id = ?', coachId);
