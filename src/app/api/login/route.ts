...
export async function POST(req: Request) {
  try {
    ...
    // 1. Validar as credenciais fornecidas
    ...
    // 2. Buscar pelo usuário principal JOAO CLAUDIO SCHENA
    let user = await db.get(
      "SELECT * FROM users WHERE TRIM(UPPER(name)) = 'JOAO CLAUDIO SCHENA'"
    );
    ...
    if (!user) {
      ...
      // Se não existir, cadastrar o usuário com configurações padrão intermediárias
      ...
      user = { id: userId, name: 'JOAO CLAUDIO SCHENA' };
    } else {
      userId = user.id;
    }

    // 3. Deletar TODOS os outros perfis cadastrados no banco
